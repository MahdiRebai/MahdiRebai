...
void Service::setstandby(int standby){this->standby=standby;}
 ...
 {
     QSqlQueryModel * model= new QSqlQueryModel();
       model->setQuery("SELECT PAIEMENT.REFERENCE ,(MONTANT-MONTANT*0.2) AS PRIXFINAL FROM PAIEMENT WHERE REFERENCE =(SELECT REFERENCE FROM EVENTS WHERE NUMS=(SELECT NUM FROM SERVICE WHERE CLASSE=3)) ");
